...
RC Db::drop_table(const char *table_name)
{
  ...
  if (filesystem::exists(table_meta_path)) filesystem::remove(table_meta_path);
  if (filesystem::exists(table_data_path)) filesystem::remove(table_data_path);
  if (filesystem::exists(table_lob_path)) filesystem::remove(table_lob_path);
  return RC::SUCCESS;
}
